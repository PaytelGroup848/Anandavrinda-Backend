...
class InvoiceService {
  ...
  async generateInvoice(orderId, generatedBy = null, type = "auto") {
    ...
    const relativePdfPath = path
      .join("uploads", "invoices", fileName)
      .replace(/\\/g, "/");
  ...
  async createPdf(order, invoiceNumber, pdfPath, settings = {}) {
    return new Promise((resolve, reject) => {
      ...
      const company = {
        name: billing.companyName || process.env.COMPANY_NAME || "Anandavrinda",
        ...
        gst: billing.gstin || tax.gstNumber || process.env.COMPANY_GST || "N/A",

        pan: billing.pan || "N/A",
      };

      // Header
      doc.fontSize(24).fillColor("#0f766e").text(company.name, 40, 35);
      ...
      doc.fontSize(22).fillColor("#111827").text("TAX INVOICE", 390, 40, {
        width: 160,
        align: "right",
      });
      ...
      // Customer
      doc.fontSize(13).fillColor("#111827").text("Bill To", 40, 160);
      ...
      // Payment info
      doc.fontSize(13).fillColor("#111827").text("Payment Details", 350, 160);
      ...
      doc
        ...
        .text(
          `Transaction: ${order.payment.transactionId || order.payment.paymentId || "-"}`,
          350,
          213,
          {
            width: 200,
      ...
      doc.rect(40, y, 515, 28).fill("#f3f4f6");
      ...
      doc.moveTo(totalX, y).lineTo(555, y).strokeColor("#d1d5db").stroke();
      ...
      doc
        ...
        .fillColor("#6b7280")
        .text(
          "Thank you for shopping with Anandavrinda. This is a computer-generated invoice.",
          40,
          760,
          {
            align: "center",
